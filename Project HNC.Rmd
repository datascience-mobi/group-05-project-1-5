...
heatmap_position = sapply(1:nrow(goi.lit), function(a) {
  ...
  if(length(position) == 0) {position[1] = NA}
  return(position)
})

goi.lit$Heatmap_position = heatmap_position # add heatmap position to goi.lit
rm(heatmap_position)
goi.lit = goi.lit[order(goi.lit$Heatmap_position),]
goi.lit

goi.SL = rownames(heatmap.data)[1:5] # choose 5 genes with most frequent deleterious mutation
...
plot(net, layout = layout1, rescale = T, ylim = c(-0.9,1), xlim = c(-0.02,-0.1), edge.color = edge.list$colours , edge.width = 2, edge.arrow.mode = 0, vertex.color = "grey", vertex.size = 10, vertex.label.font = 2, vertex.label.color = "black", main = "Network of SL interaction") 
...
```{r}
edge.list = network(SDL.partner.reduced) 
...
We want to reduce the number of partner genes for better plotting without removing genes which are connected to more than one GOI.
```{r}
...
data.frame("SL genes" = names(SL.partner),
           SL_partner = sapply(1:length(SL.partner.reduced), function(a){nrow(SL.partner.reduced[[a]])}),
           "SDL genes" = names(SDL.partner),
           SDL_partner = sapply(1:length(SDL.partner.reduced), function(a){nrow(SDL.partner.reduced[[a]])})
           )
```
