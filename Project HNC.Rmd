...
Nur gene, welche in Expression data und CNA data vorkommen
```{r}
...
g = do.call(rbind, Map(data.frame, expression=expression.norm.plusinCNA, CNA=CNA.norm.plusinexpression))

#remove CNA values below 0
g <- g[which(g$CNA >= 0),]
g <- g[which(g$expression >=0),]
#remove rows with missing values
g<- g[complete.cases(g),]

#Plotting of normalized expression valuess against CNA data. 
ggplot(g, aes(x= expression, y= CNA , color = "smooth")) + geom_point()

#k-mean of expression and CNA data


#Versuch 3
m=as.matrix(cbind(g$expression, g$CNA),ncol=2)

cl=(kmeans(m,2,iter.max = 10, nstart = 1))

g$cluster=factor(cl$cluster)
centers=as.data.frame(cl$centers)

ggplot(data=g, aes(x=expression, y=CNA, color=cluster )) + 
  geom_point() + 
