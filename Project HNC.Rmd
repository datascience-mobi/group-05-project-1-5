...
names(barplot.data) = c("Genes", "Normalised expression values")
barplot.data$Group = ifelse(barplot.data$"Normalised expression values">0,"overexpressed","underexpressed")
barplot.data = barplot.data[order(barplot.data$"Normalised expression values"),]
...
ggplot(data = HNC_mutated_genes_df_highest_ordered, aes(x=Gene, y=Freq)) +
  ...
  theme_grey()+ coord_flip()HNC_mut_plot
```
...
ggplot(comb.ex.cna[,2:3], aes(x= expression, y= copynumber , color = "smooth")) +
  geom_point()
...
## Making a linear based logistic regression
```{r 4_factor_mutation}
regression.data$mutation = factor(regression.data$mutation, levels = c("FALSE", "TRUE")) #reformat the data because strings can not be an input for a machine learning model
head(regression.data)
```
...
train.set = train.set[,2:ncol(train.set)] #get rid of the gennames becaue they are now the rownames
...
## Evaluate the model for the performance
```{r 4_test}
pred = predict(lrfit,newdata=test.set,type="response")
hist(pred) #look at the predictions
```

## Make a nice plot to evaluate the precision and the recall
```{r 4_treshhold}
