...
CNA.plusinexpression = copynumber.HNC[ which(rownames(copynumber.HNC) %in% ID2),]
rm(ID2)

#Combine Expression and CNA data in one data frame for plotting and k-mean clustering 
g = do.call(rbind, Map(data.frame, expression=expression.norm.plusinCNA, CNA=CNA.plusinexpression))
...
kmeans(g, centers, iter.max = 10, nstart = 1)
plot(g, data = NULL, class = NULL, size = 2,
  legend.position = c("right", "bottom", "left", "top", "none"),
  title = "K-Means Results", xlab = "expression",
  ylab = "CNA")
