...
'Daisy.Spearman' <- function(input_genes){
  lapply(1:length(input_genes), function(m) { # build up a list with the 5 goi, first lapply: goes through the code for each gene A
  goi = input_genes[m]
  ...
  out = data.frame(genes = rownames(expression), p_value = p.value) # return a dataframe with the columns genes(HugoSymbols) and p_values (p.values)
...
'partner' <- function(genes_input, SoF, functional.examination, coexpression){
  lapply(1:length(genes_input), function(a) {
    ...
    out = data.frame(genes = partnergenes, SoF = dat_picker.SoF[which(dat_picker.SoF$genes %in% partnergenes),"p_value"], functional_examination = dat_picker.functional_examination[which(dat_picker.functional_examination$genes %in% partnergenes),"p_value"], coexpression = dat_picker.coexpression[which(dat_picker.coexpression$genes %in% partnergenes),"p_value"])
    return(out)
  })
}
```
...
data.frame(gene = names(SL.partner), SL_partner = sapply(1:length(SL.partner), function(a){nrow(SL.partner[[a]])}))
```
Reduce SL.parter the most significant
```{r}
SL.partner[[1]] = reduce.partner(SL.partner, 1, 0.8)
SL.partner[[2]] = reduce.partner(SL.partner, 2, 0.35)
SL.partner[[3]] = reduce.partner(SL.partner, 3, 0.35) 
SL.partner[[4]] = reduce.partner(SL.partner, 4, 0.15)
data.frame(gene = names(SL.partner), SL_partner = sapply(1:length(SL.partner), function(a){nrow(SL.partner[[a]])}))
```
...
```{r}
TP53.List = SL.partner[["TP53"]]$genes
append(TP53.List, "TP53")
edge.list.TP53 = data.frame(Gene.A = "TP53",Gene.B = TP53.List, colours = "darkred")

TTN.List = SL.partner[["TTN"]]$genes
edge.list.TTN = data.frame(Gene.A = "TTN",Gene.B = TTN.List, colours = "darkgreen")

MUC16.List = SL.partner[["MUC16"]]$genes
edge.list.MUC16 = data.frame(Gene.A = "MUC16",Gene.B = MUC16.List, colours = "turquoise")

FAT1.List = SL.partner[["FAT1"]]$genes
edge.list.FAT1 = data.frame(Gene.A = "FAT1",Gene.B = FAT1.List, colours = "yellow")

CDKN2A.List = SL.partner[["CDKN2A"]]$genes
edge.list.CDKN2A = data.frame(Gene.A = "CDKN2A",Gene.B = CDKN2A.List, colours = "orange")


edge.list = bind_rows(edge.list.TP53,edge.list.TTN,edge.list.MUC16,edge.list.FAT1,edge.list.CDKN2A)
...
plot(net, layout = layout2, rescale = T, ylim = c(-0.9,1), xlim = c(-0.02,-0.1), edge.color = edge.list$colours, edge.arrow.size = 0.1 , edge.width = 2, edge.arrow.mode = 1, vertex.color = "grey", vertex.size = 8.1, vertex.label.font = 2, vertex.label.degree = -pi/2, vertex.label.color = "black" ) 

legend (x = 1.2, y = 1, goi.SL,  fill = c("darkred", "darkgreen", "turquoise", "yellow", "orange")) 

# Cleanup of the environment
rm(edge.list.CDKN2A,edge.list.FAT1,edge.list.MUC16,edge.list.TP53,edge.list.TTN,layout1,layout2,layout3,CDKN2A.List,FAT1.List,TP53.List,TTN.List,MUC16.List)
