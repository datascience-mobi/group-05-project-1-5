...
```
Genes with NA values are the same for the HNC samples compared to all the other samples. Therefore, they can be deleted in the reference data as well.
...
### Cleanup of the enviroment
```{r}
rm(kd.ceres.HNC.dist, kd.ceres.reference, kd.ceres.reference.dist, esgene1, esgeneT)
```
...
comb.ex.cna <- comb.ex.cna[which(comb.ex.cna$copynumber >= 2 & comb.ex.cna$expression >=0),]
...
kmeans = kmeans(x = comb.ex.cna[,2:3], centers = 3,iter.max = 100, nstart = 1) #see elbow method (k = 5) and apply kMeans clustering onto the dataset of the customers
...
rm(mutation.HNC.df, heatmap.genes, heatmap.samples)
```
...
```{r}
'Daisy.Wilcox' <- function(input_genes,input_data){
  lapply(1:length(input_genes), function(m) {
  ...
  mutated.ID = as.integer(mutated.ID[is.na(mutated.ID) == FALSE]) # ID of samples with goi mutation

  p.value = lapply(1:nrow(input_data), function(b) {
    mutated.cna = t(input_data[b, mutated.ID])
    not_mutated.cna = t(input_data[b, not_mutated.ID])
    p = wilcox.test(mutated.cna, not_mutated.cna, alternative = "greater")$p.value # one sided (greater) Wilcoxon Sum Rank Test 
  })

  out = data.frame(genes = rownames(input_data), p_value = do.call(rbind, p.value))
...
'Daisy.Spearman' <- function(input_genes){
  # hier kannst du noch deine Funktion einfügen Tobi
}
```
Define function to converge SL/SDL sets.
```{r}
'partner' <- function(genes_input, SoF, functional.examination, coexpression){
  lapply(1:length(genes_input), function(a) {
    ...
    out = data.frame(genes = partnergenes, SoF = dat_picker.SoF[which(SL.SoF[[a]]$genes %in% partnergenes),"p_value"], functional_examination = dat_picker.functional_examination[which(SL.functional.examination[[a]]$genes %in% partnergenes),"p_value"], coexpression = dat_picker.coexpression[which(SL.coexpression[[a]]$genes %in% partnergenes),"p_value"])
    return(out)
  })
}
```
...
```{r}
'reduce.partner' <- function(list, element_of_list, treshhold){
  list[[element_of_list]][which(list[[element_of_list]]$SoF < treshhold*max(list[[element_of_list]]$SoF) & list[[element_of_list]]$functional_examination < treshhold*max(list[[element_of_list]]$functional_examination) & list[[element_of_list]]$coexpression < treshhold*max(list[[element_of_list]]$coexpression)),]
}  
```
...
### SoF
```{r}
SL.SoF = Daisy.Wilcox(goi.mut, copynumber)
names(SL.SoF) = goi.mut
```
### Functional examination
```{r}
SL.functional.examination = Daisy.Wilcox(goi.mut, kd.ceres)
...
```{r}
SL.coexpression = lapply(1:length(goi.mut), function(m) { # build up a list with the 5 gio, first lapply: goes through the code for each gene A
  goi = goi.mut[m]

  
p.value = lapply(1:nrow(expression), function(a){  # creation of a list with the length of the expression data, for the reacent goi go through every gene and calculate cor.test
 ...
 p = cor.test(expression.goi,expression.GeneB, alternative = "greater", method = "spearman", exact = FALSE)$p.value # one sided spearman correlation test  
  ...
  out = data.frame(genes = rownames(expression), p_values = do.call(rbind, p.value)) # return a dataframe with the columns genes(HugoSymbols) and p_values (p.values)
  out = out[which(out$p_value < 0.05),] # filter for significant p.values
  out = out[which(out$genes != goi),]   # select only these wich are not the same gene as the goi (would be a corr of 1)
  return(out)
})
names(SL.coexpression) = goi.mut
...
names(SDL.SoF) = goi.SDL
```
### Functional examination
```{r}
SDL.functional.examination = Daisy.Wilcox(goi.SDL, kd.ceres)
names(SDL.functional.examination) = goi.SDL
```
### Gene coexpression
```{r}
SDL.coexpression = Daisy.Spearman(goi.SDL, expression)
names(SDL.coexpression) = goi.SDL
```
## Converging the different SL and SDL sets
for SL
```{r}
SL.partner = partner(goi.SL, SL.SoF, SL.functional.examination, SL.coexpression)
names(SL.partner) = goi.SL
lapply(1:length(SL.partner), function(a){nrow(SL.partner[[a]])})
```
```{r}

SL.partner[[1]] = reduce.partner(SL.partner, 1, 0.1)
SL.partner[[3]] = reduce.partner(SL.partner, 3, 0.6)
SL.partner[[4]] = reduce.partner(SL.partner, 4, 1) # percentage = 1 muss noch angepasst werden
SL.partner[[5]] = reduce.partner(SL.partner, 5, 1) # percentage = 1 muss noch angepasst werden
lapply(1:length(SL.partner), function(a){nrow(SL.partner[[a]])})
```
...
regression.mutation = lapply(1:length(mutation), function(a) {
  out = mutation[[a]][which(mutation[[a]]$isDeleterious == TRUE),]
  return(out)
})
names(regression.mutation) = names(mutation)
```
...
regression.data = lapply(1:length(regression.genes), function(a) {
  ...
  rg.mut = lapply(1:length(regression.mutation), function(m) {
    out = ifelse(regression.genes[a] %in% regression.mutation[[m]]$Hugo_Symbol, TRUE, FALSE)
    return(out)
  })
  rg.mut = do.call(rbind, rg.mut)
