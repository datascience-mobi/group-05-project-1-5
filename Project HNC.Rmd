...
--> warum wir nur deleterious benutzen
...
To check if  there are more essential genes in HNC, the menas of HNC patients and the reference group were compared.
...
#boxplot
boxplot(sample(esgeneT,20, replace = FALSE), xlab = "gene", horizontal =F, main="Knockdown values of 20 random genes",#sample =random pick
        xlab="genes",
...
km.data = lapply(1:length(ID), function(x) {
  ...
  return (out)#Create data rame with expression and CNA values
})
#Combine rows for k mean clustering
comb.ex.cna = do.call(rbind, km.data)
comb.ex.cna = comb.ex.cna[which(comb.ex.cna$copynumber >= 1 & comb.ex.cna$expression >=quantile(comb.ex.cna$expression,0.95)),]# Filter for most important values
```

To generate an overview of the corresponding expression and CNA values a ggplot was executed
```{r 1_plotexpCNA}
...
In order to set the optimal ammount of clusters in kmeans the ellbow method was executed
```{r 1_ellbowmethod}
...
plot(1:10,
     ...
     ylab = 'EMP')
```
The ellbow test shows that 3 clusters are sufficient.

To visualize centers in expression and CNA data, kmeans clustering was executed.
```{r 1_Visualizing k-means}
...
heatmap(data.matrix(heatmap.data)[1:40,], Rowv=NA, Colv=NA, col = c("lightgrey", "tomato2"), scale="column", cexRow = 0.5, cexCol = 0.8, main = "Most frequently deleterious mutated genes in HNC samples")
```
...
#plotting parameters are defined. vertex= Genes edges = interactions
  plot(net, layout = layout1, rescale = T, ylim = c(-0.9,1), xlim = c(-0.1,-0.1), edge.color = edge.list$colours , edge.width = width, edge.arrow.mode = 0, vertex.color = node.color,vertex.frame.color = frame.color, vertex.size = node.size, vertex.label.font = 2, vertex.label.color = "black", vertex.label = label, margin = 0.1)

#adding a legend to the plot
  legend (x = 1.2, y = 1, GOI,  fill = c("darkred", "darkgreen", "turquoise", "yellow", "orange"))
...
regression.expression = expression.HNC[regression.genes,]
regression.copynumber = copynumber.HNC[regression.genes,]
regression.kd.ceres = kd.ceres.HNC[regression.genes,]
```
...
regression.data = lapply(1:length(regression.genes), function(a) {
  rg.expr = t(regression.expression[a, ID[1]])
  rg.cna = t(regression.copynumber[a, ID[1]])
  rg.kd = t(regression.kd.ceres[a, ID[1]])
  rg.mut = ifelse(regression.genes[a] %in% mutation[[ID[1]]]$Hugo_Symbol, TRUE, FALSE)
...
model.results = predict(train.model, newdata = subset(testing.set.rg, select=c("expression","copynumber","kockdown")), type = "response")
...
## Plot the data
```{r 4_plot}
# pdf(paste0(root.dir, "/", "190711_LinearModel_LogRegression_PrecisionOverview.pdf"), width = 15)
