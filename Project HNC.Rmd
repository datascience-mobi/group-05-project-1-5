...
'Daisy.Spearman' <- function(input_genes){
  ...
  p.value = sapply(1:20, function(a){  # creation of a list with the length of the expression data, for the reacent goi go through every gene and calculate cor.test #!! nrow(expression)
  ...
  out = data.frame(genes = rownames(expression)[1:20], p_value = p.value) # return a dataframe with the columns genes(HugoSymbols) and p_values (p.values) #!! 1:20
