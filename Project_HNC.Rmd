...
![Most affected tissues and organs by head and neck cancer](images/HNC_Kopf.png)
...
![Daisy-Workflow](images/Daisy.png)
...
## Milestones
![Milestones of our project](images/milestones.png)
...
# Generating a function that searches all elements a of our mutation data set which are isDeleterious
'mutation.clean' = function(data){
  cleaned_data = lapply(1:length(data), function(a) { # Data will be mutation (length of 544) or mutation.HNC (length of 27)
    dat_picker = data[[a]] # Pick the element of the list on position 'a'
    dat_picker = dat_picker[, c("Hugo_Symbol", "isDeleterious")] # Choose only the columns with gene name and whether they have a deleterious mutation
    dat_picker = dat_picker[which(dat_picker$isDeleterious == TRUE),] # Choose only deleterious mutations
    return(dat_picker)
  })
  names(cleaned_data) = names(data) # Keep the names of the input data
  return(cleaned_data)
}
# Apply the new function on our two mutation data sets in order to clean them
...
# Boxplot of 20 of knockdown values from random genes
boxplot(sample(esgeneT,20, replace = FALSE), horizontal =F, main="Knockdown values of 20 random genes",
        ylab="Knockdown values",
        col="blue",
        border="brown",
        outline = TRUE,
        notch = TRUE,
        las = 2) #Flip Xlab names
