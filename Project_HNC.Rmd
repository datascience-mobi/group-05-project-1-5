...
## Milestones
```{r, echo=FALSE, fig.cap="Milestones of our project", out.width='100%'}
...
head(all_mutated_genes_highest_30_ordered)
```
...
```{r 1_muation_frequency5}
ID_HNC_highest_frequencies = head(order(HNC_mutated_genes_df[["Freq"]], decreasing = TRUE), length(ID))
HNC_mutated_genes_df_highest_ordered = HNC_mutated_genes_df[ID_HNC_highest_frequencies, ]
rownames(HNC_mutated_genes_df_highest_ordered) = 1:length(ID)
...
```{r  1_muation_frequency6, fig.height = 7}
...
```{r  1_muation_frequency7, fig.height = 7}
...
### Cleanup of the enviroment
```{r 1_muation_cleanup}
rm(all_mutations, all_mutated_genes, all_mutated_genes_df, ID_all_highest_30_frequencies,
   ID_all_highest_frequencies, all_mutated_genes_df_highest_30,
   all_mutated_genes_df_highest_30_ordered, colnames_ordered, HNC_mutations,
   HNC_mutated_genes, HNC_mutated_genes_df, ID_HNC_highest_frequencies,
   HNC_mutated_genes_df_highest_ordered)
```
...
```{r, echo=FALSE, fig.cap="Genes with frequent and highly significant somatic genetic changes in HNSCC", out.width='100%'}
...
SL.SoF = Daisy.Wilcox(goi.SL, copynumber, inactive_overactive = 0, less_greater = 1)
...
SL.functional.examination = Daisy.Wilcox(goi.SL, kd.ceres, inactive_overactive = 0, less_greater = 0)
...
SDL.SoF = Daisy.Wilcox(goi.SDL, copynumber, inactive_overactive = 1, less_greater = 1)
```
Functional examination
```{r 3_SDL_functional_examination}
# Functional examination, overactive, one sided Wilcoxon rank test, alternative = "less"
SDL.functional.examination = Daisy.Wilcox(goi.SDL, kd.ceres, inactive_overactive = 1, less_greater = 0)
```
Gene coexpression
...
train.set = train.set[,2:ncol(train.set)]
...
```{r 4_cleanup}
rm(precision, pred, recall, regression.genes, threshhold_ls, inTrain, lrfit, results_df,
   test.set, threshold_list, train.set)
```
